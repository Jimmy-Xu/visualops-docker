...
fn_test() {
  ...
  show_title "${NO}.3 test scribed(write log)"
  fn_exec 'docker run -it --rm --link madeiracloud-scribe:scribe madeiracloud/scribe:centos6 bash -c "echo `date` hello world! | scribe_cat -h \$SCRIBE_PORT_1463_TCP_ADDR:\$SCRIBE_PORT_1463_TCP_PORT test"'
  check_rlt $? "connect failed"
...
##########################################
show_title "1 start build madeiracloud/scribe:centos6-base"
OLD_IMG=$(docker images | grep "^madeiracloud/scribe.*centos6-base " | awk '{print $3}')
time fn_exec 'docker build -t madeiracloud/scribe:centos6-base .'
check_rlt $? "build failed"
NEW_IMG=$(docker images | grep "^madeiracloud/scribe.*centos6-base " | awk '{print $3}')
...
##########################################
fn_test_default 3 "madeiracloud/scribe:centos6-base"

if [ "${IMG_UPDATED}" == "true" ];then
  ##########################################
  show_title "4.1 start optimize image size for madeiracloud/scribe:centos6-base"
  fn_exec 'docker run --name madeiracloud-scribe madeiracloud/scribe:centos6-base bash'
  check_rlt $? "start temp container failed"

  show_title "4.2 export image(madeiracloud/scribe:centos6-base) to export.tar"
  fn_exec 'docker export madeiracloud-scribe > export.tar'
  check_rlt $? "export image to tar failed"

  show_title "4.3 import image(madeiracloud/scribe:centos6-optimized) from export.tar"
  ls -l export.tar
  fn_exec 'cat export.tar | docker import - madeiracloud/scribe:centos6-optimized'
  check_rlt $? "import image from tar"

  show_title "4.4 build madeiracloud/scribe:centos6 from madeiracloud/scribe:optimized"
  cd optimize
  time fn_exec 'docker build -t madeiracloud/scribe:centos6 .'
  check_rlt $? "build madeiracloud/scribe:centos6 failed"
  cd ..

  show_title "4.5 show image for madeiracloud/scribe:centos6"
  fn_exec 'docker images | grep madeiracloud/scribe.*centos6'
...
##########################################
fn_test 5 "madeiracloud/scribe:centos6" "/visualops/conf/scribe.conf"
